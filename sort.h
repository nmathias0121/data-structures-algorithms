...
// Shell sort algorithm -> variation of insertion sort
// Time Complexity : O(n^2)
// Params : array, size of array
template <typename T>
int shell_sort(T arr[], size_t arr_size)
{
    // Start with a big gap, then halve the gap for each iteration
    for (int gap = arr_size/2; gap > 0; gap /= 2)
    {
        ...
        for (int i = gap; i < arr_size; i += 1)
        {
            ...
            int j;         
            for (j = i; j >= gap && arr[j - gap] > gap_element; j -= gap)
                arr[j] = arr[j - gap];
